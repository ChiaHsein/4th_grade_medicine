...
mysqli_query($connect,"SET NAMES 'UTF8'"); //各種設定
    ...
    session_start();
	$pre_balance=$_SESSION['pre_balance'];
	//echo $pre_balance;
	//引入函式庫
	...
	foreach($sheetData as $key => $col){  //讀取每一列
		...
		if($rowindex>=1){ //前面武行不讀取
			...
			array_push($data,array($text[0],$text[1]));	 //二維
			//列的index歸零
	...
	for($Key=count($data)-1;$Key>=0;$Key--) { 
		...
		//echo $data[$Key][0]." ".$data[$Key][1]."</br>";
	 
		$sql="UPDATE `".$pre_balance."` SET `pre_balance`=".$data[$Key][1].",`t_balance` = `pre_balance` + `income` - `consum`,`profit_loss`=`total`-`t_balance`  where `drug_code`='".$data[$Key][0]."';";
