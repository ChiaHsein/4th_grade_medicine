...
$get_month=date("Ymd",mktime(date('H')+6, date('i'), date('s'), date('m'), date('d'), date('Y')));
...
foreach ($worksheet->getRowIterator() as $row) {
	...
	array_push($data,array($text[1],$text[5]));	 //二維
	
	$sql="UPDATE `".$table_name."` SET `consum`=".$text[5].",`t_balance`=`pre_balance`+`income`-`consum`,`profit_loss`=`total`-`t_balance` where `drug_code`='".$text[1]."';";
